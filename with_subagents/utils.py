...
# Import necessary types for Google Generative AI
...
# Mark these functions as async because they use await
async def update_interaction_history(session_service, app_name, user_id, session_id, entry):
    ...
    try:
        # Get current session
        # FIX: Await get_session
        session = await session_service.get_session(
            app_name=app_name, user_id=user_id, session_id=session_id
        )

        # Get current interaction history
        # Ensure session and session.state are valid before accessing
        if not session or not hasattr(session, 'state'):
            print(f"Error: Could not retrieve valid session or state for session_id: {session_id}")
            return
...
# These functions call update_interaction_history, so they don't need to be async themselves,
# but the functions they call (update_interaction_history) now are.
...
# Mark this function as async because it uses await
async def display_state(
    ...
    try:
        ...
        if not session or not hasattr(session, 'state'):
            print(f"Error: Could not retrieve valid session or state for session_id: {session_id}")
            print("-" * (22 + len(label))) # Print closing line even on error
            return
...
async def process_agent_response(event):
    ...
    if event.content and event.content.parts:
        for part in event.content.parts:
            # Check if the part has a 'text' attribute and it's not empty/whitespace
            if hasattr(part, "text") and part.text and not part.text.isspace():
                print(f"  Text: '{part.text.strip()}'")
                has_specific_part = True # Mark that we found a text part

    # Check for final response after specific parts
    final_response = None
    if event.is_final_response(): # This check is important to only process the final output
        if (
            event.content
            and event.content.parts
            # Accessing the first part, assuming it's the main content
...
# This function MUST be async because it uses 'await' for 'runner.run_async' and 'process_agent_response'
# It also calls add_user_query_to_history and add_agent_response_to_history,
# which now internally await other async functions.
async def call_agent_async(runner, user_id, session_id, query):
    """Call the agent asynchronously with the user's query."""
    # Ensure 'types' is correctly imported and used for Content
    ...
    if final_response_text and agent_name:
        # add_agent_response_to_history calls update_interaction_history, which is now async.
        # When calling an async function from a sync context, you'd typically use asyncio.run or similar,
        # but here, add_agent_response_to_history is being called within an already async function (call_agent_async),
        # so it should work fine if the internal calls to async functions are handled correctly.
        # However, the original structure was:
        #   call_agent_async (async) -> add_agent_response_to_history (sync) -> update_interaction_history (async)
